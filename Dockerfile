...
FROM --platform=$BUILDPLATFORM mcr.microsoft.com/dotnet/sdk:9-alpine AS build
...
RUN --mount=type=cache,id=nuget,target=/root/.nuget/packages \
    dotnet publish apps/api/src/Preznt.Api/Preznt.Api.csproj \
    -a ${TARGETARCH/amd64/x64} --use-current-runtime --self-contained false -o /app
...
ENTRYPOINT ["dotnet", "Preznt.Api.dll"]
